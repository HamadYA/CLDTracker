...
class Category_embedding(nn.Module):
    ...
    def __init__(self):
        ...
        # if not os.path.exists(sequence_name_path) or not os.path.exists(dataset_name_file):
        if os.path.exists(sequence_name_path) or os.path.exists(dataset_name_file):

            print("CLDTracker")

            # Read sequence names from the file
            with open(sequence_name_path, 'r') as file:
                sequence_name = [line.strip() for line in file.readlines()]

            # Read dataset names from the file
            with open(dataset_name_file, 'r') as file:
                dataset_name = [line.strip() for line in file.readlines()]
            ...
            for seq, dataset in zip(sequence_name, dataset_name):
                if dataset == 'trackingnet':
                    try:
                        description_encodings = {seq: trackingnet_json[seq]}
                    except:
                        description_encodings = {"l9QgZQGtQWI_0": trackingnet_json['l9QgZQGtQWI_0']}
                
                elif dataset == 'lasot':
                    description_encodings = {seq: lasot_json[seq]}

                elif dataset == 'got10k':
                    description_encodings = {seq: got10k_json[seq]}
                
                elif dataset == 'coco':
                    description_encodings = {seq: coco_json[seq]}

                
                for key in description_encodings:
                    la = description_encodings[key]

                final_list.append(la)
            
            
            max_length = max(len(sublist) for sublist in final_list)
            extended_final_list = [
                sublist * (max_length // len(sublist)) + sublist[:max_length % len(sublist)]
                for sublist in final_list
            ]
            tokenized_sublists = [torch.cat([clip.tokenize(f"a photo of a {v}.") for v in sublist]) for sublist in extended_final_list]
            self.text_inputs = torch.stack(tokenized_sublists).to(self.device)
        ...
        for i in range(batch_size):
            if self.training:
                ...
                with torch.no_grad():
                    ...
                    # Bag
                    tem_logits = self.logit_scaling * tem_image_features @ self.text_features[i].t()
                    tem_similarity = (100.0 * tem_image_features @ self.text_features[i].T.half()).softmax(dim=-1)
                    tem_values, tem_indices = tem_similarity[0].topk(1)
                    tem_class_label = self.text[i][tem_indices]
                    ...
                    # Bag
                    sem_logits = self.logit_scaling * sea_image_features @ self.text_features[i].t()
                    sem_similarity = (100.0 * sea_image_features @ self.text_features[i].T.half()).softmax(dim=-1)
                    sem_values, sem_indices = sem_similarity[0].topk(1)
                    sem_class_label = self.text[i][sem_indices]
                    ...
                    # class_attention = -torch.norm(tem_logits - torch.stack(sem_class_ttfum).mean(dim=0))
                    # attention = self.softmax(torch.Tensor([class_attention])).unsqueeze(0)
                ...
                if self.tem_image_features is None:
                    with torch.no_grad():
                        ToPILImage = transforms.ToPILImage()
                        template1 = ToPILImage(template[i])
                        
                        # tem_image_input = self.preprocess(template).unsqueeze(0).to(self.device)
                        tem_image_input = self.preprocess(template1).unsqueeze(0).to(self.device)
                ...
                ToPILImage = transforms.ToPILImage()
                search1 = ToPILImage(search[i])
                # sea_image_input = self.preprocess(search).unsqueeze(0).to(self.device)
                sea_image_input = self.preprocess(search1).unsqueeze(0).to(self.device)
                with torch.no_grad():
                    sea_image_features = self.clip_model.encode_image(sea_image_input)
                    x = self.adapter(sea_image_features)
                    sea_image_features = self.ratio * x + (1 - self.ratio) * sea_image_features
