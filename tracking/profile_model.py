...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='cldtrack', choices=['cldtrack'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='vitb_384_mae_ce_32x4_ep300', help='yaml configure file name')
    args = parser.parse_args()

    return args


def evaluate_vit(model, template, search, template_clip, search_clip):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, template_clip, search_clip),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, template_clip, search_clip)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, template_clip, search_clip)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "cldtrack":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_cldtrack
        ...
        # CLIP's template and search
        to_pil = transforms.ToPILImage()
        template_clip = to_pil(torch.rand([3, 224, 224]))
        search_clip = to_pil(torch.rand([3, 224, 224]))
        ...
        if merge_layer <= 0:
            evaluate_vit(model, template, search, template_clip, search_clip)
        else:
            evaluate_vit_separate(model, template, search, template_clip, search_clip)
